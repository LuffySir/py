...
def getLdaModel(path):
    ...
    docTopic.extend(ldamodel.get_document_topics((bow for bow in corpus),0.5))
    print(docTopic)
    #return ldamodel,textsAll

def outputSentiPro(path,ldaPath):
    ldamodel,textsAll = getLdaModel(path)
    filelist = os.listdir(path)
    for i in filelist:
...
# 所有文本连接成字符串
file_str = getAllStr(path_test)
# 获取字符串中包含的情感词（存在killing->ill的情况，后面需要注意）
sentiWord = getSentiWordStemmer(path_senti)  # 487个
#outputSentiPro(path_test,path_test_lda)
getLdaModel(path_test)
